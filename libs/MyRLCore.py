...
class Agent(object):
    ...
    def init_fit_parallel(self, nb_steps=10000, sampler_update_interval=500,action_repetition=1, callbacks=None, verbose=1,
        ...
        self.training_callbacks = [] # if not self.training_callbacks else self.training_callbacks[:]
        if callbacks:
            self.training_callbacks += callbacks

        if verbose == 1:
            self.training_callbacks += [TrainIntervalLogger(interval=log_interval)]
        elif verbose > 1:
    ...
    def fit_parallel(self, experience_list, sampler_update_interval=500, n_backward_calls=1):
        done = False

        if self.episode_step == 0 and len(experience_list) > 0: # start of a new "episode"
        ...
        try:
            ...
            for e in experience_list:
                ...
                self.training_callbacks.on_step_end(self.episode_step, step_logs)
                self.episode_step += 1
                self.step += 1
            ...
            if done:
                ...
                self.training_callbacks.on_episode_end(self.episode, episode_logs)

                print("Main Thread: episode: %d, # new experiences: %d, backward/experience: %.2f, backward/sec: %.2f, backward/fit: %d, backward/ep: %d" 
                    % (self.episode + 1, self.episode_step, float(self.episode_n_backward_calls)/self.episode_step, float(self.episode_n_backward_calls)/self.episode_backward_time.total_seconds(), n_backward_calls, self.episode_n_backward_calls))
