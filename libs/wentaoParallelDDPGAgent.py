...
from MyRLCore import Agent
from wentaoMemory import *
...
class ParallelDDPGAgent(Agent, Model):
    """Write me
    """
    def __init__(self, nb_actions, actor, critic, critic_action_input, memory,
        ...
        super(ParallelDDPGAgent, self).__init__(**kwargs)
    ...
    def load_weights(self, filepath):
        ...
        self.update_target_models_hard()
        #try:V
        #    memory_filepath = filepath + '_memory'
    ...
    #
    #TODO-PARALLEL: this method would do nothing.
    # this would be performed by the parallel runner.
    #
    ...
    # state0, action, reward, state1, terminal1 = experiences[0]
    def backward(self, experiences):
        # Store most recent experience in memory.
        #TODO-PARALLEL: we will get a list of episodes
        # from the parallel runner manager and dump them into the memory
        # Since each episode is self contained, we don't need a strict ordering
        # if self.step % self.memory_interval == 0:
        #     newEpisode = Experience(state0=np.array([self.recent_observation]), 
        #                             action=self.recent_action, 
        #                             reward=reward,
        #                             state1=np.array([newobs]), 
        #                             terminal1=terminal)
        for e in experiences:
            self.memory.append(e)

        #print "Memory len: " +  str(len(self.memory.episodes))
        ...
        if can_train_either and self.step % self.train_interval == 0:
            state0_batch, action_batch, reward_batch ,state1_batch, terminal1_batch = \
                 self.memory.sample(self.batch_size)

            # Prepare and validate parameters.
            state0_batch = np.expand_dims(state0_batch, 1)
            state0_batch = self.process_state_batch(state0_batch)
            
            reward_batch = reward_batch.squeeze()

            state1_batch = np.expand_dims(state1_batch, 1)
            state1_batch = self.process_state_batch(state1_batch)

            terminal1_batch = terminal1_batch.squeeze()
